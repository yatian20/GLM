...
SILR <- function(A,k,kw,family){
  ...
  for(t in 1:T){
    ...
    grad0_t <- grad0_t + diag(diag(grad0_t))
  ...
  for(i in 1:999){
    ...
    for(t in 1:T){
      ...
      grad1_t <- grad1_t + diag(diag(grad1_t))
...
#full version of SILR (include the projection step and one-step)
SILR2 <- function(A,k,kw,family){
  ...
  for(t in 1:T){
    ...
    grad0_w[[t]] <- grad0_t %*% W0[[t]]
    W1[[t]] <- W0[[t]] + eta_wt * grad0_w[[t]]
    W1[[t]] <- t(apply(W1[[t]],1,function(x) x/max(sqrt(sum(x^2)/2.5),1)))
    grad0_z <- grad0_z + grad0_t %*% Z0
  }
  Z1 <- Z0 + eta_z * grad0_z
  Z1 <- t(apply(Z1,1,function(x) x/max(sqrt(sum(x^2)/2.5),1)))
  ...
  #Stage 4: one-step for Z and Wt
  est <- Newton(A,Z1,W1,family)
  return(list(Z_init = Zr,W_init = Wr,Z_hat = est$Z_hat,W_hat = est$W_hat))
}

#theory version of SILR (use pseudo liklihood)
SILR3 <- function(A,k,kw,family){
  n <- dim(A)[1]
  T <- dim(A)[3]
  if(family == 'poisson'){
    g <- function(x) exp(x)
  ...
  for(i in 1:999){
    ...
    grad1_w <- list()
    for(t in 1:T){
      grad1_t <- A[,,t] - g(Z1 %*% t(Zr) + W1[[t]] %*% t(Wr[[t]]))
      grad1_w[[t]] <- grad1_t %*% Wr[[t]]
      eta_wt <- -sum(diag(t(W1[[t]]-W0[[t]]) %*% (grad1_w[[t]]-grad0_w[[t]])))/sum(diag(t(grad1_w[[t]]-grad0_w[[t]]) %*% (grad1_w[[t]]-grad0_w[[t]])))
      if(is.na(eta_wt))
        break
      W0[[t]] <- W1[[t]]
...
Newton <- function(A,Z,W,family){
  ...
  if(family == 'poisson'){
    nu_p <- function(x) exp(x)
    nu_pp <- function(x) exp(x)
  }
  if(family == 'bernoulli'){
    nu_p <- function(x) 1/(1+exp(-x))
...
MASE <- function(A,k){
  ...
  U <- NULL
  for(t in 1:T)
    U <- cbind(U,eigen((A[,,t] + t(A[,,t]))/2)$vectors[,1:k])
  U <- svd(U)$u[,1:k]
  
  Lam <- NULL
  for(t in 1:T)
    Lam[[t]] <- t(U) %*% A[,,t] %*% U
  return(list(U_hat = U,Lam_hat = Lam))
}                  
